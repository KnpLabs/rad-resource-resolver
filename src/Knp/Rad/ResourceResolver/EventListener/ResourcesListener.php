...
final class ResourcesListener
{
     ...
     */
    public function __construct(Parser $parser, Caster $caster, Resolver $resolver, Container $container)
    {
        $this->parser    = $parser;
        $this->caster    = $caster;
    ...
    public function resolveResources(FilterControllerEvent $event)
    {
        ...
        $resources = $this->getResourcesConfigurations($request);

        foreach ($resources as $name => $resource) {
            $resource = $this->resolver->resolve($resource);

            if ($resource->isRequired() && null === $resource) {
